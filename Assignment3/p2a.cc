...
// 	std::cout << (i32)tcp.GetLength() << std::endl;
	enqueue.open("/home/will/Homework/Assignment3/p2a.csv", std::ios::app);
...
//	std::cout << "dr," <<Simulator::Now().GetSeconds() <<","<<tcp.GetDestinationPort()-4999<<","<<tcp.GetSequenceNumber() << std::endl;
	...
	dropped.open("/home/will/Homework/Assignment3/p2a.csv", std::ios::app);
...
void CheckQueueSize(Ptr<QueueDisc> queue) {
	...
	queuesize.open("/home/will/Homework/Assignment3/p2a-queuesize.csv", std::ios::app);
...
int main(int argc, char* argv[]) {
	...
	enqueue.open("/home/will/Homework/Assignment3/p2a.csv");
...
/*
	...
	dropped.close();
*/
	queuesize.open("/home/will/Homework/Assignment3/p2a-queuesize.csv");
...
//					bottleneckLink);				//specify the link between the blue circles
	...
	onetoA.SetChannelAttribute ("Delay", StringValue("1ms"));
	...
	twotoA.SetChannelAttribute ("Delay", StringValue("4ms"));
	...
	threetoA.SetChannelAttribute ("Delay", StringValue("5ms"));
...
//	OnOffHelper sourceHelperOnetoA("ns3::TcpSocketFactory", InetSocketAddress (onetoAip.GetAddress(0), 4000));
	...
	AddressValue remoteAddressOnetoA (InetSocketAddress (AtoBip.GetAddress(1), port));
...
//	AddressValue remoteAddressTwotoSix (InetSocketAddress (BtoFiveip.GetAddress(1), port));
	AddressValue remoteAddressTwotoSix (InetSocketAddress (AtoBip.GetAddress(1), port+1));
...
//	AddressValue remoteAddressThreetoSeven (InetSocketAddress (BtoFiveip.GetAddress(1), port));
	AddressValue remoteAddressThreetoSeven (InetSocketAddress (AtoBip.GetAddress(1), port+2));
...
//	AddressValue remoteAddressFourtoEight (InetSocketAddress (BtoFiveip.GetAddress(1), port));
	AddressValue remoteAddressFourtoEight (InetSocketAddress (AtoBip.GetAddress(1), port+3));
...
//	ApplicationContainer sinks;
	...
	Address sinkLocalAddressFive(InetSocketAddress (AtoBip.GetAddress(1), port));
	PacketSinkHelper sinkHelperFive ("ns3::TcpSocketFactory", sinkLocalAddressFive);
	sinkFive = sinkHelperFive.Install(nodes.Get(5));

	ApplicationContainer sinkFiveTwo;
	Address sinkLocalAddressFiveTwo(InetSocketAddress (AtoBip.GetAddress(1), port+1));
	PacketSinkHelper sinkHelperFiveTwo ("ns3::TcpSocketFactory", sinkLocalAddressFive);
	sinkFiveTwo = sinkHelperFiveTwo.Install(nodes.Get(5));

	ApplicationContainer sinkFiveThree;
	Address sinkLocalAddressFiveThree(InetSocketAddress (AtoBip.GetAddress(1), port+2));
	PacketSinkHelper sinkHelperFiveThree ("ns3::TcpSocketFactory", sinkLocalAddressFiveThree);
	sinkFiveThree = sinkHelperFiveThree.Install(nodes.Get(5));

	ApplicationContainer sinkFiveFour;
	Address sinkLocalAddressFiveFour(InetSocketAddress (AtoBip.GetAddress(1), port+3));
	PacketSinkHelper sinkHelperFiveFour ("ns3::TcpSocketFactory", sinkLocalAddressFiveFour);
	sinkFiveFour = sinkHelperFiveFour.Install(nodes.Get(5));
