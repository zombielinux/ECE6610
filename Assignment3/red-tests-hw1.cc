 ...
 *
 */
//Tinkered with and commented WS 10-3-17
  ...
  if ( (test == 1) || (test == 3) )
    {
      ...
      ApplicationContainer sinkApp = sinkHelper.Install (n3n5.Get (1));
      ...
      AddressValue remoteAddress
        (InetSocketAddress (i0i5.GetAddress (1), port));
...
//Originally started at 3 seconds after simulation start WS 10-4
...
/*      uint16_t port1 = 50001;
      ...
      ApplicationContainer sinkApp1 = sinkHelper1.Install (n3n5.Get (1));
...
/*      uint16_t port3 = 50003;
      ...
      ApplicationContainer sinkApp3 = sinkHelper3.Install (n0n5.Get (0));
...
/*      uint16_t port4 = 50004;
      ...
      ApplicationContainer sinkApp4 = sinkHelper4.Install (n1n5.Get (0));
...
//All this is is a layer 7 application on a node sending/receiving data
//FTP connection originating at node 0, sinking to node 2
/*      OnOffHelper clientHelper1 ("ns3::TcpSocketFactory", Address ());
      clientHelper1.SetAttribute ("OnTime", StringValue ("ns3::ConstantRandomVariable[Constant=1]"));
      clientHelper1.SetAttribute ("OffTime", StringValue ("ns3::ConstantRandomVariable[Constant=0]"));
      clientHelper1.SetAttribute 
	...
	//change from i5 to i6, has to be set for all remoteAddress
        (InetSocketAddress (i3i5.GetAddress (1), port1));
...
*/
      // Connection #2
//FTP connection originating at node 1, sinking to node 2
/*      OnOffHelper clientHelper2 ("ns3::TcpSocketFactory", Address ());
      clientHelper2.SetAttribute ("OnTime", StringValue ("ns3::ConstantRandomVariable[Constant=1]"));
      clientHelper2.SetAttribute ("OffTime", StringValue ("ns3::ConstantRandomVariable[Constant=0]"));
      clientHelper2.SetAttribute 
...
*/
      // Connection #3
//FTP connection originating at node3, sinking to node 4
/*     OnOffHelper clientHelper3 ("ns3::TcpSocketFactory", Address ());
      ...
      AddressValue remoteAddress3
        (InetSocketAddress (i0i5.GetAddress (0), port3));
      clientHelper3.SetAttribute ("Remote", remoteAddress3);
      clientApps3.Add (clientHelper3.Install (n3n5.Get (1)));
      clientApps3.Start (Seconds (3.5));
      clientApps3.Stop (Seconds (client_stop_time));
*/
      // Connection #4
//TELNET connection originating at node3, sinking to node 5
/*      OnOffHelper clientHelper4 ("ns3::TcpSocketFactory", Address ());
      ...
      AddressValue remoteAddress4
        (InetSocketAddress (i1i5.GetAddress (0), port4));
...
//  n3n5 = NodeContainer (c.Get (3), c.Get (5));
 ...
 n0n5 = NodeContainer (c.Get (0), c.Get (4));
 n1n5 = NodeContainer (c.Get (1), c.Get (4));
 n2n5 = NodeContainer (c.Get (2), c.Get (4));
 n3n5 = NodeContainer (c.Get (3), c.Get (4));
...
//Create link speed and latency for the two nodes on the near side of the network. 

  p2p.SetQueue ("ns3::DropTailQueue");
  p2p.SetDeviceAttribute ("DataRate", StringValue ("100Mbps"));
  p2p.SetChannelAttribute ("Delay", StringValue ("1ms"));
  NetDeviceContainer devn0n5 = p2p.Install (n0n5);
...
*/
//Create link speed and latency for the two nodes that join the two halves
//Note that the node parameters are defined higher up in this file. 
/*
  p2p.SetQueue ("ns3::DropTailQueue");
...
*/
  QueueDiscContainer queueDiscs = tchRed.Install (devn0n5);

//Create link speed and latency for the two nodes on the far side of the network
...
//Create network definition between node 0 and node 2. Similar syntax for all the other nodes. 
// /24 networks, not defining individual hosts. 
...
//Set RED Queue min_th and max_th values. 
